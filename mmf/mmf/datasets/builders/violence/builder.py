...
from mmf.datasets.builders.violence.dataset import (
    violenceFeaturesDataset,
...
@registry.register_builder("violence")
class violenceBuilder(MMFDatasetBuilder):
    def __init__(
        self,
        dataset_name="violence",
        dataset_class=violenceImageDataset,
        *args,
        **kwargs
    ):
        super().__init__(dataset_name, dataset_class, *args, **kwargs)
        self.dataset_class = violenceImageDataset

    @classmethod
    def config_path(self):
        return "configs/datasets/violence/defaults.yaml"

    def load(self, config, dataset_type, *args, **kwargs):
        config = config
        
        if config.use_features:
            self.dataset_class = violenceFeaturesDataset
