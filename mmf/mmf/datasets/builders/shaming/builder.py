...
from mmf.datasets.builders.shaming.dataset import (
    shamingFeaturesDataset,
    shamingImageDataset,
)
...
@registry.register_builder("shaming")
class shamingBuilder(MMFDatasetBuilder):
    def __init__(
        self,
        dataset_name="shaming",
        dataset_class=shamingImageDataset,
        *args,
        **kwargs
    ):
        super().__init__(dataset_name, dataset_class, *args, **kwargs)
        self.dataset_class = shamingImageDataset

    @classmethod
    def config_path(self):
        return "configs/datasets/shaming/defaults.yaml"

    def load(self, config, dataset_type, *args, **kwargs):
        config = config
        
        if config.use_features:
            self.dataset_class = shamingFeaturesDataset
