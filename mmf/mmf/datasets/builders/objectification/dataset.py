...
class objectificationFeaturesDataset(MMFDataset):
    def __init__(self, config, *args, dataset_name="objectification", **kwargs):
...
class objectificationImageDataset(MMFDataset):
    def __init__(self, config, *args, dataset_name="objectification", **kwargs):
        super().__init__(dataset_name, config, *args, **kwargs)
        assert (
            self._use_images
...
def generate_prediction(report):
    ...
    for idx, image_id in enumerate(report.id):
        ...
        predictions.append({"id": image_id.item(), "label": label})
    return predictions
