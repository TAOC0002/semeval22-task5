...
from mmf.datasets.builders.mami.dataset import (
    mamiFeaturesDataset,
    mamiImageDataset,
)
...
@registry.register_builder("mami")
class mamiBuilder(MMFDatasetBuilder):
    def __init__(
        self,
        dataset_name="mami",
        dataset_class=mamiImageDataset,
        *args,
        **kwargs
    ):
        super().__init__(dataset_name, dataset_class, *args, **kwargs)
        self.dataset_class = mamiImageDataset

    @classmethod
    def config_path(self):
        return "configs/datasets/mami/defaults.yaml"

    def load(self, config, dataset_type, *args, **kwargs):
        config = config
        
        if config.use_features:
            self.dataset_class = mamiFeaturesDataset
