...
class mamiFeaturesDataset(MMFDataset):
    def __init__(self, config, *args, dataset_name="mami", **kwargs):
    ...
    def __getitem__(self, idx):
       ...
       # comment out for multi-task and prediction
        
        if "label" in sample_info:
            current_sample.targets = torch.tensor(
                sample_info["label"], dtype=torch.long
            )
        elif ("misogynous" in sample_info):
            misogynous = torch.tensor(
                sample_info["misogynous"], dtype=torch.long
            )        
            shaming = torch.tensor(
                sample_info["shaming"], dtype=torch.long
            )        
            stereotype = torch.tensor(
                sample_info["stereotype"], dtype=torch.long
            )        
            objectification = torch.tensor(
                sample_info["objectification"], dtype=torch.long
            )        
            violence = torch.tensor(
                sample_info["violence"], dtype=torch.long
            )     

            current_sample.targets = torch.tensor([misogynous, shaming, stereotype, objectification, violence])
...
class mamiImageDataset(MMFDataset):
    def __init__(self, config, *args, dataset_name="mami", **kwargs):
        super().__init__(dataset_name, config, *args, **kwargs)
        assert (
            self._use_images
...
#     # labels = (scores > 0).to(torch.int32)
#     labels = scores
# #     for i in range(len(labels)):
# #         if labels[i][0] == 0:
# #             labels[i] = torch.full((5,), 0, dtype=torch.int32)

# #     one = (scores[:,0] > 0.0622).unsqueeze(axis=1)
# #     two = (scores[:,1] > -0.23).unsqueeze(axis=1)
# #     three = (scores[:,2] > -0.271).unsqueeze(axis=1)
# #     four = (scores[:,3] > 0.039).unsqueeze(axis=1)
# #     labels = torch.cat((one, two, three, four), axis = 1).to(torch.int32)
            
#     # --------------------------------------------- #

#     for idx, image_id in enumerate(report.id):
#         answers = labels[idx]
#         predictions.append({"id": str(image_id.item())+'.jpg', "misogynous": answers[0].item(), "shaming": answers[1].item(), 
#                             "stereotype": answers[2].item(), "objectification": answers[3].item(), "violence": answers[4].item()})
#     return predictions
...
def generate_prediction(report):
    ...
    for idx, image_id in enumerate(report.id):
        ...
        predictions.append({"id": image_id.item(), "proba": proba})
    return predictions
