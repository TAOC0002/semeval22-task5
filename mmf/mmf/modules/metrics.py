...
@registry.register_metric("scoreA")
class ScoreA(BaseMetric):
    """Metric for calculating F1. Can be used with type and params
    argument for customization. params will be directly passed to sklearn
    f1 function.
    **Key:** ``f1``
    ...
    def __init__(self, *args, **kwargs):
        super().__init__("scoreA")
        
    def check_matrix(self, matrix, gold, pred):
    ...
    def compute_f1(self, pred_values, gold_values):
        matrix = metrics.confusion_matrix(gold_values, pred_values)
        matrix = self.check_matrix(matrix, gold_values, pred_values)
    ...
    def extract_field(self, truth, submission, index):
        gold = []
        guess = []
    ...
    def compute_scoreA(self, truth, submission):
        gold, guess = self.extract_field(truth, submission, 0)
        score = self.compute_f1(guess, gold)
        return score
...
#        # multi_task learning
#         labels = (scores > 0)
#         expected = expected.to(torch.bool)
        
#         for i in range(len(labels)):
#             if labels[i].to(torch.int32).sum() > 0:
#                 labels[i][0] = True

        labels = scores.argmax(axis=-1, keepdims = True).to(torch.bool)
        expected = torch.unsqueeze(expected.to(torch.bool), axis=1)
        
        # ------------------------------------------------------ #
        
        for index in range(len(labels)):
            submission[str(index)] = labels[index].tolist()
            truth[str(index)] = expected[index].tolist()

        scorea = self.compute_scoreA(truth=truth, submission=submission)

        return expected.new_tensor(scorea, dtype=torch.float)
    
    
    
    
@registry.register_metric("scoreB")
class ScoreB(BaseMetric):
    """Metric for calculating F1. Can be used with type and params
    argument for customization. params will be directly passed to sklearn
    f1 function.
    **Key:** ``f1``
    ...
    def compute_scoreB(self, truth, submission):
        results = []
        total_occurences = 0
        
        for index in range(1,5):
            gold, guess = self.extract_field(truth, submission, index)
            f1_score = self.compute_f1(guess, gold)
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        for index in range(len(labels)):
            if labels[index][0] == False:
                labels[index] = torch.full((5,), False, dtype=torch.bool)
            submission[str(index)] = labels[index].tolist()
            truth[str(index)] = expected[index].tolist()

        scoreb = self.compute_scoreB(truth=truth, submission=submission)

        return expected.new_tensor(scoreb, dtype=torch.float)

import time
@registry.register_metric("time")
class Time(BaseMetric):
    """Metric for calculating Micro Average Precision.
    ...
    **Key:** ``micro_ap``
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        expected = sample_list["targets"]
        return expected.new_tensor(time.time()%1000000, dtype=torch.float)