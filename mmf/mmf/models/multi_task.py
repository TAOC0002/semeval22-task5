...
#         self.clf1 = nn.Sequential(BertPredictionHeadTransform(self.bert.config),
#                                   nn.Linear(self.bert.config.hidden_size, 100),
#                                   nn.Linear(100, 10),
#                                   nn.Linear(10, 1)
#                                  )
#         self.clf2 = nn.Sequential(BertPredictionHeadTransform(self.bert.config),
#                                   nn.Linear(self.bert.config.hidden_size, 100),
#                                   nn.Linear(100, 10),
#                                   nn.Linear(10, 1)
#                                  )
#         self.clf3 = nn.Sequential(BertPredictionHeadTransform(self.bert.config),
#                                   nn.Linear(self.bert.config.hidden_size, 100),
#                                   nn.Linear(100, 10),
#                                   nn.Linear(10, 1)
#                                  )
#         self.clf4 = nn.Sequential(BertPredictionHeadTransform(self.bert.config),
#                                   nn.Linear(self.bert.config.hidden_size, 100),
#                                   nn.Linear(100, 10),
#                                   nn.Linear(10, 1)
#                                  )
#         self.clf5 = nn.Sequential(BertPredictionHeadTransform(self.bert.config),
#                                   nn.Linear(self.bert.config.hidden_size, 100),
#                                   nn.Linear(100, 10),
#                                   nn.Linear(10, 1)
#                                  )
...
#                                  )
    ...
    def init_weights(self):
        ...
        # ------------------------------------------------------------------------------- # 
            ...
            self.clf1.apply(self.bert._init_weights)
            self.clf2.apply(self.bert._init_weights)
            self.clf3.apply(self.bert._init_weights)
            self.clf4.apply(self.bert._init_weights)
            self.clf5.apply(self.bert._init_weights)
            
           
        # Set last hidden layer
        if "losses" in self.config and self.config.zerobias:
            for loss in self.config.losses:
                if "bce_with_logits" in loss["type"]:
                    for layer in range(1,2):
                        self.clf1[layer].bias.data.fill_(self.config.biasfill)
                        self.clf2[layer].bias.data.fill_(self.config.biasfill)
                        self.clf3[layer].bias.data.fill_(self.config.biasfill)
                        self.clf4[layer].bias.data.fill_(self.config.biasfill)
                        self.clf5[layer].bias.data.fill_(self.config.biasfill)
    ...
    def forward(
        ...
        misogynous = self.clf1(logits).contiguous().view(-1, 1)       
        shaming = self.clf2(logits).contiguous().view(-1, 1)
        stereotype = self.clf3(logits).contiguous().view(-1, 1)
        objectification = self.clf4(logits).contiguous().view(-1, 1)
        violence = self.clf5(logits).contiguous().view(-1, 1)
        
        output_dict["scores"] = torch.cat((misogynous, shaming, stereotype, objectification, violence), dim=1)
...
@registry.register_model("multi_task")
class MultiTask(BaseModel):
    ...
    @classmethod
    def config_path(cls):
        return "configs/models/multi_task/defaults.yaml"
